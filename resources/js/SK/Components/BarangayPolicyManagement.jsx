...
const BarangayPolicyManagement = () => {
  ...
  const fetchPolicies = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`/api/barangay-policies?archived=${viewArchived ? 'true' : 'false'}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        showNotification('Barangay policy updated successfully!', 'success');
      } else {
        response = await axios.post('/api/barangay-policies', data, config);
        showNotification('Barangay policy created successfully!', 'success');
      }
